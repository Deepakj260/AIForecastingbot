...
if uploaded_file:
    ...
    # Calculate adjusted trend-based growth for Firm Schedule
    df_part_sorted = df_part.sort_values("Month")
    df_recent = df_part_sorted[df_part_sorted['Month'] >= (df_part_sorted['Month'].max() - pd.DateOffset(months=24))]
    df_recent = df_recent.reset_index(drop=True)
    firm_slope = 0
    if len(df_recent) >= 2:
        df_recent['Month_Index'] = range(len(df_recent))
        coeffs = np.polyfit(df_recent['Month_Index'], df_recent['Firm Schedule Qty'], deg=1)
        firm_slope = coeffs[0] / df_recent['Firm Schedule Qty'].mean()
    ...
    # Forecast Firm Schedule based on linear trend slope from history
    st.subheader("📈 Forecast Firm Schedule and Lifting (Trend Adjusted from Historical Pattern)")
    ...
    firm_forecast_df['Firm Schedule Qty'] = [base_firm * (1 + firm_slope * (i+1)) for i in range(24)]
    ...
    firm_forecast_df['Seasonal_Firm_Avg'] = df_part['Firm Schedule Qty'].mean()
    ...
    ax.set_title("24-Month Forecast Based on 2-Year Firm Schedule Trend")
    ax.set_xlabel("Month")
