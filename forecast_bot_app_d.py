...
if uploaded_file:
    ...
    # Calculate CAGR based on actual lifting, not firm schedule
    df_part_sorted = df_part.sort_values("Month")
    initial_val = df_part_sorted.iloc[0]['Actual Lifting Qty']
    final_val = df_part_sorted.iloc[-1]['Actual Lifting Qty']
    ...
    # Forecast Firm Schedule Qty for next 24 months using Actual Lifting CAGR
    st.subheader("📈 Forecast Firm Schedule and Lifting for Next 24 Months (CAGR on Actual + Seasonality + Holiday)")
    ...
    base_actual = df_part.iloc[-1]['Actual Lifting Qty']
    firm_forecast_df['Firm Schedule Qty'] = [(base_actual * ((1 + part_cagr) ** (i+1))) for i in range(24)]

    # Prepare features for lifting prediction
    last_known = df_part.iloc[-1]
    firm_forecast_df['Firm_Lag1'] = base_actual
    firm_forecast_df['Actual_Lag1'] = base_actual
    ...
    ax.set_title("24-Month Forecasted Firm Schedule and Lifting with Actual CAGR")
    ax.set_xlabel("Month")
